...
- **Auto-savestate (slot AUTO)** — optional writes to `_auto.svs` on sleep/suspend, exit, and ROM switch (Continue), and before loading another savestate (Undo — reload AUTO to reverse a bad load). Emulator Options → Auto-savestate; default on. Main-thread only (not from the power callback). Loading a ROM can prompt to restore that slot. Also available as slot AUTO in the savestate submenu (main-menu numeric spinner remains 0–9).
